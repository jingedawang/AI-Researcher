...
def process_proposals(cache_name, arxiv_id, conference, step):
    ...
    json_objects = load_json_files(f"../experiment_result/ours_v2/{conference}/{arxiv_id}/{step}_final_proposal_wi_method_decom/")
    if not json_objects:
    ...
    with open(f'../cache_results_test/final_proposals/{cache_name}/{step}.json', 'w') as f:
